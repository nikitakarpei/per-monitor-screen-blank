...
export class GnomeMonitorProvider {
    ...
    listMonitors() {
        ...
        const count =
            typeof display?.get_n_monitors === 'function'
                ? display.get_n_monitors()
                : 0;
        const primary =
            typeof display?.get_primary_monitor === 'function'
                ? display.get_primary_monitor()
                : 0;
        const monitorManager =
            globalThis.backend?.get_monitor_manager?.() ??
        ...
        for (let index = 0; index < count; index += 1) {
            ...
            if (!managerMonitor) {
                logInfo(
                    'no manager monitor at display index; monitor skipped',
                    { index, managerCount: managerMonitors.length },
                );
                continue;
            }

            const connector = String(
                managerMonitor.get_connector?.() ?? '',
            ).trim();
            ...
            if (!runtimeSpec.isStable) {
                logInfo(
                    'runtime monitor missing stable hardware identity; monitor returned unstable',
                    {
                        index,
                        connector,
                        vendor: String(
                            managerMonitor.get_vendor?.() ?? '',
                        ).trim(),
                        product: String(
                            managerMonitor.get_product?.() ?? '',
                        ).trim(),
                        serial: String(
                            managerMonitor.get_serial?.() ?? '',
                        ).trim(),
                    },
                );
            }
