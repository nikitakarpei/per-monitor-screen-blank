...
export function normalizeKeepAwakeMinutes(
    minutes,
    fallback = DEFAULTS.keepAwakeMinutes,
) {
    return Math.max(1, Number.isFinite(minutes) ? minutes : fallback);
}

function shouldRearmKeepAwake(
    mode,
    keepAwakeMinutes,
    lastMode,
    lastKeepAwakeMinutes,
) {
    return (
        mode === 'keep-awake' &&
        (mode !== lastMode || keepAwakeMinutes !== lastKeepAwakeMinutes)
    );
}

export function resolveSettingsModeEffect(
    mode,
    keepAwakeMinutes,
    lastMode,
    lastKeepAwakeMinutes,
    ...
    if (
        ...
        return {
            ...
            keepAwakeMs: keepAwakeMinutes * 60 * 1000,
        };
    }
